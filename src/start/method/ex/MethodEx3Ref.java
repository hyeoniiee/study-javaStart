...
public class MethodEx3Ref {
    ...
    public static int deposit(int balance, int amount) { // 압금 메서드 생성
        balance += amount;
        System.out.println(amount + "원을 입금하였습니다. 현재 잔액: " + balance + "원");
        return balance;
    }
    public static int withdraw(int balance, int amount) { // 출금 메서드 생성
        if (balance >= amount) {
            balance -= amount;
...
입금과 출금 부분이 메서드로 명확하게 분리되었기 때문에 이후에 변경 사항이 발생하면 관련된 메서드만 수정하면 된다.
