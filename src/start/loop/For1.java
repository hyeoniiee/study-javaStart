...
/*
`for` 문도 `while` 문과 같은 반복문이고, 코드를 반복 실행하는 역할을 한다. `for` 문은 주로 반복 횟수가 정해져 있을 때 사용한다.
    <for 문 구조>
...
`for` 문은 다음 순서대로 실행된다.
1. 초기식이 실행된다. 주로 반복 횟수와 관련된 변수를 선언하고 초기화 할때 사용한다. 초기식은 딱 1번 사용된다.
...
`for` 문은 복잡해 보이지만 `while` 문을 조금 더 편하게 다룰 수 있도록 구조화 한 것 뿐이다.
 */
